...
Tiles are **real-time** widgets (closest snapshot at the selected time — cameras and Infiray thermals today, text logs later) or **graph** widgets (the selected window, centered on that time — audio today, current/voltage traces later).
...
3. **Timeline** — live by default. Click to scrub, scroll to zoom the window. Real-time tiles (camera, thermal) show the closest snapshot at the window centre; graph tiles (audio) show the selected window centered on that timestamp.
