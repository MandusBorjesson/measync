...
2. **Record / Stop** — one toggle. First Record copies the last ~5 s of preview into the capture ring, then keeps appending. Record after Stop resumes the same take. Oldest capture samples drop (time-aligned across sources) when the RAM cap is reached. Click the RAM meter to change the cap. **Reset** discards RAM (confirm if unsaved) and returns to the 5 s preview. After Stop, **Play** / **Pause** and **speed** replay the frozen take in this browser (cameras, graphs, and microphone audio follow the playhead; 1× plays the recorded sound). Those controls are grayed out during preview and recording.
3. **Lock back / Lock front** — pin the window to the oldest or newest samples. Both on (default) fits the full buffer. Scroll to zoom; drag the timeline or a graph to pan (turns locks off). **plot** in the header sets how many points each graph draws (default 100; stored in this browser). **Cursor** / **Span** place named measurement markers on every graph (and the timeline): a cursor prints values at one time like the red playhead; a span reports min/max/avg/integral over a time delta. Drag the handle at the top of a bar to move it. **Markers** lists them (jump / delete). Real-time tiles (camera) show the closest snapshot at the window centre, or a live image while the buffer is growing and lock front is on; graph tiles (audio, Joulescope) always show the selected window (mean line, min/max band when samples collapse; time ticks; playhead values while scrubbing); hybrid tiles (thermal) do both, and you can drag zones on the feed for local min/max traces.
